...
class BlogFavoriteController extends Controller
{
    ...
    public function index(Request $request)
    {
        return $this->queryResult(function () use ($request) {
            $model = Blog_Favorite::where('user_id', $this->_user->getAuthIdentifier());
    ...
    public function store(Request $request)
    {
        return $this->withTransactionNotNotify(function () use ($request) {
            $exists = Blog_Favorite::where("blog_id", $request['blog_id'])
                ->where('user_id', $this->_user->getAuthIdentifier())->first();
            if (is_null($exists)) {
                $store = Blog_Favorite::make($request->all());
